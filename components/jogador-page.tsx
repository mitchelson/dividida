...
interface JogadorPageClientProps {
...
export function JogadorPageClient({
  initialProfile,
  userEmail = "",
  matchesPlayed = 0,
  recentGames = [],
  isOwnProfile = false,
}: JogadorPageClientProps) {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 px-4 py-4 space-y-4">
              ...
              <div className="space-y-2">
                {recentGames.slice(0, 5).map((g) => (
                  <Link
                    key={g.id}
                    href={`/jogo/${g.id}`}
                    className="flex items-center justify-between py-1.5 border-b border-border last:border-0 hover:bg-muted/50 -mx-1 px-1 rounded transition-colors"
                  >
                    <div>
                      <p className="text-sm font-medium text-foreground">{g.name}</p>
                      <p className="text-xs text-muted-foreground">
                        {new Date(g.game_date + "T12:00:00").toLocaleDateString("pt-BR", {
                          day: "2-digit",
                          month: "short",
                        })}
                      </p>
        ...
        {/* Account Info - Only show if viewing own profile */}
        {isOwnProfile && (
          <Card>
            <CardHeader className="p-4 pb-2">
              <CardTitle className="text-sm flex items-center gap-2">
        ...
        {/* Edit Prompt - Only show if viewing own profile */}
        {isOwnProfile && (
